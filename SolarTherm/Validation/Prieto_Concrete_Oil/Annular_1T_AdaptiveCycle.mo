within SolarTherm.Validation.Prieto_Concrete_Oil;
...
model Annular_1T_AdaptiveCycle "Concrete storage perforated with steel pipes. Thermal oil (DowthermA) is used to charge and discharge the TES."
  ...
  //Thermal Load
  //parameter SI.MassFlowRate m_flow_steam = 210.0 * 1.0e3 / 3600.0 "Mass Flow Rate of required steam to digestion (kg/s)"; //210.0 t/h
  //parameter SI.Temperature T_steam_inlet = 340.0 + 273.15 "Required supply temperature of hot steam (K)"; //340 degC
  //parameter SI.Pressure p_steam_inlet = 105.0 * 1.0e5 "Required supply temperature of hot steam (Pa)"; //105 bar
  //parameter SI.Temperature T_condensate = 85.0 + 273.15 "Return temperature of condensate (K)"; //85.0 degC
  //parameter SI.Pressure p_condensate = 1.40 * 1.0e5 "Return pressure of condensate (Pa)"; //1.4 bar
  
  //parameter Real f_blowdown = 0.05 "Blowdown fraction of the boiler (-)";
  //parameter Real eff_boiler = 0.90 "Overall efficiency of the boiler (-)";
  //parameter Real eff_pump = 0.80 "Pumping efficiency of the boiler HX pump (-)";
  /*
  parameter SI.SpecificEnthalpy h_steam_inlet = Modelica.Media.Water.IF97_Utilities.h_pT(p_steam_inlet,T_steam_inlet);
  parameter SI.SpecificEnthalpy h_condensate = Modelica.Media.Water.IF97_Utilities.h_pT(p_condensate,T_condensate);
  parameter SI.SpecificEnthalpy h_economiser = Modelica.Media.Water.IF97_Utilities.hl_p(p_steam_inlet);
  
  parameter SI.HeatFlowRate Q_flow_econ = (1.0/eff_boiler)*(m_flow_steam/(1.0-f_blowdown))*(h_economiser-h_condensate);
  parameter SI.HeatFlowRate Q_flow_evap_sh = (1.0/eff_boiler)*m_flow_steam*(h_steam_inlet - h_economiser);
  
  parameter SI.HeatFlowRate Q_flow_load = Q_flow_econ + Q_flow_evap_sh;
  parameter SI.HeatFlowRate Q_flow_load_old = m_flow_steam*(h_steam_inlet-h_condensate);
  
  parameter SI.SpecificEnthalpy h_fluid_econ_in = h_f_process_des - Q_flow_evap_sh/m_flow_process_des;
  parameter Medium.ThermodynamicState state_fluid_econ_in_des = Medium.setState_phX(Medium.p_default, h_fluid_econ_in) "Thermodynamic state of air at the minimum system temperature T_min.";
  parameter SI.Temperature T_fluid_econ_in = Medium.temperature(state_fluid_econ_in_des);
  */
  
  //BoilerHX Thermal Load Modelica
  //SolarTherm.Models.Fluid.HeatExchangers.BoilerHX_Calculations.Set_Inlet_Outlet Boiler_HX(redeclare package Medium = Medium, f_blowdown = f_blowdown, eff_boiler = eff_boiler, eff_pump = eff_pump, m_flow_H2O_out = m_flow_steam, T_H2O_out = T_steam_inlet, p_H2O_out = p_steam_inlet, T_H2O_return = T_condensate, p_H2O_return = p_condensate, T_fluid_in = T_process_des, T_fluid_out = T_min);
  
  //These parameters are varied
  parameter Real HM = 2.0 "Heater Multiple";
  parameter SI.Time t_discharge = 1.235*3600.0*4 "Rated discharging period (s)";
  ...
  parameter SI.Length d_pipe = 0.05 "Hole diameter in the filler (m)";
  parameter SI.Length d_solid = 0.1241 "Equivalent solid diameter per repeating unit (m)";
  parameter SI.Length L_pipe = 72.0 "Length of TES in the charging/discharging axis (m)";
  ...
  parameter SI.Temperature T_max = 393.0 + 273.15 "Maximum temperature (K)";
  parameter SI.Temperature T_process_des = 350.0 + 273.15 "Design process inlet temperature (K)";
  parameter SI.Temperature T_high_set = 350.0 + 273.15 "TES hot blend temperature temperature (K)";
  parameter SI.Temperature T_process_min = 350.0 + 273.15 "Minimum tolerated outlet temperature to process (K)";
  parameter SI.Temperature T_heater_max = 336.0 + 273.15 "Maximum tolerated outlet temperature to heater (K)";
  parameter SI.Temperature T_low_set = 336.0 + 273.15 "TES cold blend temperature (K)";
  parameter SI.Temperature T_heater_des = 293.0 + 273.15 "Design receiver inlet temperature (K)";
  parameter SI.Temperature T_min = 278.15 + 273.15 "Minimum temperature (K)";
  ...
  //Tank Geometry
  parameter SI.MassFlowRate m_flow_process_des = Q_process_des/(h_f_process_des - h_f_min);//Boiler_HX.m_flow_fluid;
  ...
  parameter SI.Power Q_process_des = 411.0e6;//Boiler_HX.Q_flow_duty_total;//m_flow_process_des*(h_f_process_des - h_f_min) "Design required process heat-rate (W_th)";
